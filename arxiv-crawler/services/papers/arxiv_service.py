...
class ArxivService:
    ...
    async def fetch_papers(self, category: str, max_results: int = 10):
        logging.info(f"Fetching papers for category: {category} with max results: {max_results}")
        try:
            params = {
            ...
            logging.info(f"Parsed Atom feed for category: {category}")
            papers = []
            ...
            logging.error(f"Error fetching papers for category {category}: {e}")
            return []
        except Exception as e:
            logging.error(f"Unexpected error fetching papers for category {category}: {e}")
            return []
