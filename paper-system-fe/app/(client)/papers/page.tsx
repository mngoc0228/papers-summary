...
function getVisiblePages(currentPage: number, totalPages: number) {
  ...
  if (currentPage >= totalPages - 1) return [totalPages - 2, totalPages - 1, totalPages].filter((p) => p > 0);
  return [currentPage - 1, currentPage, currentPage + 1];
}

function usePaperPagination() {
  ...
  useEffect(() => {
    ...
    async function loadPapers() {
      ...
      try {
        ...
        const response = (await apiRequest(url, { cache: "no-store" })) as PapersResponse;
  ...
  return { currentPage, selectedTopicId, papers, hasNextPage, loading, totalPages, updateFilters };
}

export default function PapersPage() {
  const { currentPage, selectedTopicId, papers, hasNextPage, loading, totalPages, updateFilters } = usePaperPagination();
  const visiblePages = getVisiblePages(currentPage, totalPages);
  const [topics, setTopics] = useState<{ id: string | number; name: string }[]>([]);
  
  useEffect(() => {
    apiRequest(`/topics`, { cache: "no-store" }).then((res: any) => {
      setTopics(res.data ?? []);
    }).catch(() => setTopics([]));
  }, []);
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col gap-6 border-b pb-10">
        <div className="space-y-3">
          <Badge variant="outline" className="w-fit bg-zinc-50 text-zinc-500 border-zinc-200">
            <Sparkles className="h-3 w-3 mr-2" /> Thư viện tri thức
          </Badge>
          <h1 className="text-4xl font-black tracking-tighter text-zinc-950">Thư viện bài báo</h1>
          <p className="text-zinc-500 text-base max-w-2xl leading-relaxed">
            Tiếp cận những nghiên cứu khoa học mới nhất, được tóm tắt súc tích giúp bạn tối ưu hóa thời gian nghiên cứu.
          </p>
        </div>
      ...
      {/* Topics Filter Section với hiệu ứng Scrollbar on Hover */}
      <div className="group relative">
        <div 
          className="flex items-center gap-2 overflow-x-auto pb-3 flex-nowrap
            /* Ẩn scrollbar mặc định */
            scrollbar-none [&::-webkit-scrollbar]:h-1.5
            [&::-webkit-scrollbar-track]:bg-transparent
            [&::-webkit-scrollbar-thumb]:bg-transparent
            /* Hiện scrollbar khi hover vào vùng group */
            group-hover:[&::-webkit-scrollbar-track]:bg-zinc-100
            group-hover:[&::-webkit-scrollbar-thumb]:bg-zinc-300
            group-hover:[&::-webkit-scrollbar-thumb]:rounded-full
        ...
        >
          ...
          {topics.map((topic) => (
            <Button
              key={topic.id}
              variant={selectedTopicId === String(topic.id) ? "default" : "ghost"}
              size="sm"
              className="rounded-full px-5 h-8 text-xs shadow-none shrink-0"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 min-h-[400px]">
        {loading ? (
          <div className="col-span-full text-center py-20 text-zinc-400">Đang tải bài báo...</div>
        ) : papers.length > 0 ? (
          papers.map((paper) => (
            <Card key={paper.id} className="group border-zinc-200/60 shadow-none hover:border-zinc-400 transition-all duration-300 bg-white flex flex-col rounded-xl overflow-hidden">
              <CardHeader className="p-5 space-y-4">
                <div className="flex items-center justify-between w-full">
                  <div className="flex flex-wrap gap-2 justify-start">
                    {paper.topics?.map((topic) => (
                      <Badge key={topic.name} variant="secondary" className="w-fit bg-zinc-100 text-zinc-600 border-none font-bold text-[10px] uppercase px-2 py-0.5">
                        {topic.name}
                      </Badge>
                    ))}
                  </div>
                ...
                <CardTitle className="text-lg font-bold leading-snug group-hover:text-zinc-600 transition-colors line-clamp-2 min-h-[50px]">
                  <Link href={`/papers/${paper.id}`} className="hover:underline">{paper.title}</Link>
                </CardTitle>
              </CardHeader>
          ...
          <div className="col-span-full text-center py-20 text-zinc-400">Không tìm thấy bài báo nào cho chủ đề này.</div>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-4 pt-10 border-t border-zinc-100">
        <Button
          variant="ghost"
          size="sm"
          className={currentPage > 1 ? "text-zinc-900 text-xs font-bold" : "text-zinc-300 text-xs cursor-not-allowed"}
        ...
        <Button
          variant="ghost"
          size="sm"
          className={hasNextPage ? "text-zinc-900 text-xs font-bold" : "text-zinc-300 text-xs cursor-not-allowed"}
